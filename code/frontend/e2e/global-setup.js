...
function liberarPorta(porta) {
  ...
  if (pidsNaPorta(porta).length > 0) {
    throw new Error(`Porta ${porta} ainda ocupada após taskkill — encerre o artisan serve manualmente.`)
  }
}
...
export default async function globalSetup() {
  ...
  // Matar serve ANTES do migrate evita "relation notificacoes does not exist" no poll do Bell.
  liberarPorta(8000)
  ...
  // Sobe o API aqui: no Windows o webServer do Playwright às vezes não sobe após o kill.
